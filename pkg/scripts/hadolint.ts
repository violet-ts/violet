...
const hadolintRun = async ({tag,cwd,ignores,filepath,trustedRegistries, local}:HadolintRunParams): Promise<HadolintRunResult> => {
  const ignoresArgs = ignores.flatMap(ignore => ['--ignore', ignore]);
  const trustedRegistriesArgs = trustedRegistries.flatMap(registry => ['--trusted-registry', registry]);
...
const main = async ({only, dir, hadolintTag, local}: Params): Promise<void> => {
  ...
  const files = allFiles.filter(f => only.length === 0 || onlyFrom.some(from => f.startsWith(from)))
  const results = await Promise.all(files.map(async (file) => {
    ...
    return {
      filepath,
      ...await hadolintRun({tag: hadolintTag, filepath, ignores, trustedRegistries, cwd, local}),
    };
  }))
